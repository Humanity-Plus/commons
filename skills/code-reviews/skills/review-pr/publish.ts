 ...
 *   --pages <owner/name> Host the report by committing it into a Pages-publishing
 *                        repo (one whose workflow deploys root *.html to GitHub
 *                        Pages) and link that Pages URL in the review body. The
 ...
 *   --report <path>      Report file for --gist/--pages (default: <findings dir>/report.html)
...
export function gistGuardError(visibility: string, force: boolean): string | null {
  ...
  return (
    ...
    `paths) outside the repo's access controls. Use --pages <owner/name> with a ` +
    `Pages-publishing repo whose visibility you control, host the report yourself and ` +
    `pass --report-url, or override deliberately with --force-gist.`
  );
}
...
// Human-readable footer stamp: DD.MM.YYYY - HH:MM (UTC). Falls back to the raw
// value when it isn't a parseable date.
...
export function reviewFooter(meta: Report["meta"], login = ""): string {
  const date = meta.generatedAt ? ` · ${fmtStamp(meta.generatedAt)}` : "";
...
export function buildReview(
  ...
  // System-recap block (already marker-wrapped; GitHub renders its mermaid in
  // review bodies) — the recap channel for PRs whose description we don't own.
  if (opts.recapMd) body.push(opts.recapMd.trim());
...
// Deterministic, collision-free filename for a report in a pages repo.
export function pagesReportName(repo: string, pr: string, headSha?: string): string {
  const slug = repo.replace("/", "-").replace(/[^\w.-]/g, "_");
  const sha = (headSha ?? "").slice(0, 7);
  return `review-${slug}-pr${pr}${sha ? `-${sha}` : ""}.html`;
}

// Host the HTML report by committing it into a Pages-publishing repo (the
// private-repo alternative to --gist: a repo whose Pages workflow deploys root
// *.html files, with Pages visibility controlling who can read them — on
...
async function hostOnPagesRepo(
  ...
  const existing = await gh(["api", `repos/${pagesRepo}/contents/${name}`, "-q", ".sha"]).catch(() => "");
  const body = JSON.stringify({
    message: `review-pr report: ${name}`,
    content,
    ...(existing ? { sha: existing } : {}),
  });
  await gh(["api", "-X", "PUT", `repos/${pagesRepo}/contents/${name}`, "--input", "-"], body);
  return new URL(name, htmlUrl.endsWith("/") ? htmlUrl : `${htmlUrl}/`).toString();
}
...
if (import.meta.main) {
  ...
  if (!findingsPath) {
    console.error(
      "usage: bun run publish.ts <findings.json> [--pr N] [--repo owner/name] [--report-url URL] [--gist] [--pages owner/name] [--recap] [--report path] [--min-severity sev] [--body-only] [--dry-run]"
    );
    process.exit(1);
  }
  ...
  if (opts["pages"]) {
    const pagesRepo = String(opts["pages"]);
    if (!/^[\w.-]+\/[\w.-]+$/.test(pagesRepo)) {
      console.error(`--pages expects owner/name, got "${pagesRepo}"`);
      process.exit(1);
    }
    const reportPath = (opts["report"] as string) || join(dirname(findingsPath), "report.html");
    const visibility = await gh(["repo", "view", repo, "--json", "visibility", "-q", ".visibility"]).catch(
  ...
  let recapMd: string | undefined;
  if (opts["recap"]) {
    const recapPath = join(dirname(findingsPath), "system-recap.md");
    recapMd = await Bun.file(recapPath)
      .text()
      .catch(() => "");
    if (!recapMd) {
      console.error(`--recap: no ${recapPath} — generate it first (bun run recap.ts <findings.json>).`);
      process.exit(1);
    }
  }
  ...
  const buildOpts = { reportUrl, recapMd, bodyOnly: !!opts["body-only"], minSeverity, login };
